...
class ConfigDialog(wx.Dialog):
	def __init__(self, parent, title):
		...
		self.vbox.Add(
			self.buttons, flag=wx.ALIGN_CENTER | wx.TOP | wx.BOTTOM, border=10
		)
...
class AIChatApp(wx.Frame):
	...
	def InitUI(self):
		...
		self.model_button.SetAcceleratorTable(
			wx.AcceleratorTable([(wx.ACCEL_CTRL, ord("M"), self.model_button.GetId())])
		)
		...
		vbox.Add(
			hbox_buttons, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, border=10
		)
		panel.SetSizerAndFit

	def OnManageModels(self, event):
		dialog = ModelManagerDialog(
			self, "Manage Models", self.ollama, self.current_model
		)
		if dialog.ShowModal() == wx.ID_OK:
	...
	def stream_response(self, user_input):
		...
		response = self.ollama.chat(
			model=self.current_model, messages=self.messages, stream=True
		)
		self.streaming = True
		output = ""
	...
	def load_models(self):
		...
		for model in models:
			index = self.list_ctrl.InsertItem(
				self.list_ctrl.GetItemCount(), model["name"]
			)
